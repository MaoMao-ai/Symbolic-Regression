...
def generate_function():
    """ 生成 a * log(b * x + 1) + c 形式的函数 """
    x = sp.Symbol('x')
    # 参数范围调整
    a = random.uniform(-2, 2)    # 控制函数的缩放
    b = random.uniform(0.1, 5)   # 必须为正数，控制x的缩放
    c = random.uniform(-2, 2)    # 控制垂直平移
    return a, b, c, a * sp.log(b * x + 1) + c
...
class CoefficientDataset(Dataset):
    def __init__(self, functions):
        ...
        for func in tqdm(functions):
            if isinstance(func, tuple):
                ...
                try:
                    ...
                    # 使用更密集的采样点，注意对数函数的定义域
                    sample_points = np.linspace(0, 5, 50)  # 修改采样范围为正数
                    ...
                    # f'(x) = a*b/(bx+1)
                    # 在 x=0 处，f'(0) = a*b
                    deriv_at_zero = derivatives[0]
                    
                    # 使用两点法估计 a
                    x1, x2 = sample_points[0], sample_points[-1]
                    y1, y2 = values[0], values[-1]
                    a = (y2 - y1) / (np.log(b * x2 + 1) - np.log(b * x1 + 1))
                    
                    # 使用导数估计 b
                    b = deriv_at_zero / a if abs(a) > 1e-6 else 1.0
        ...
        for i, (param, (min_val, max_val)) in enumerate(self.ranges.items()):
            if param == 'b':
                # 对b使用对数归一化，因为它是正数
                self.coefficients[:, i] = torch.log(self.coefficients[:, i] / min_val) / torch.log(torch.tensor(max_val / min_val))
            else:
...
def vae_loss(recon_x, x, mu, logvar, beta=0.1, epoch=0):
    # 动态权重
    scale_weight = min(2.0 + epoch/100, 4.0)
    
    # MSE损失
    recon_loss_a = F.mse_loss(recon_x[:, 0], x[:, 0]) * scale_weight  # 缩放参数a的权重
    recon_loss_b = F.mse_loss(recon_x[:, 1], x[:, 1])  # b参数（已经对数归一化）
...
def plot_reconstruction(model, dataset, num_samples=5, domain=(0, 5)):
    ...
    with torch.no_grad():
        ...
        for i, ax in enumerate(axes):
            ...
            distance = compute_l2_distance(orig_poly, recon_poly, domain=domain)
...
def coefficients_to_function(coeffs):
    ...
    # 对b使用指数反归一化
    b = float(COEFF_STATS['b'][0] * np.exp(
        np.clip(coeffs[1], 0, 1) * np.log(COEFF_STATS['b'][1] / COEFF_STATS['b'][0])
    ))
    c = float(COEFF_STATS['c'][1] * np.arctanh(np.clip(coeffs[2], -0.99, 0.99)))
    
    return a * sp.log(b * x + 1) + c
...
def main():
    ...
    # 修改文件名以反映新的函数类型
    dataset_file = 'Datasets/log_x+1_dataset.pkl'
    ...
    save_model(model, 'Model_Library/log_x+1_vae_model.pt')
