...
def generate_function():
    """ 生成 a * log(b * x^2 + 1) + c 形式的函数，定义域为[-5, 5] """
    x = sp.Symbol('x')
    # 参数范围调整
    a = random.uniform(-2, 2)    # 控制函数的缩放
    b = random.uniform(0.1, 1)   # 必须为正数，但要调小以适应更大的x^2范围
    c = random.uniform(-2, 2)    # 控制垂直平移
    return a, b, c, a * sp.log(b * x**2 + 1) + c
        ...
        for func in tqdm(functions):
            if isinstance(func, tuple):
                ...
                try:
                    ...
                    a = (y2 - y1) / (np.log(b * x2**2 + 1) - np.log(b * x1**2 + 1))
                    
                    # 使用导数估计 b
                    b = (deriv_at_one * (1 + 1)) / (2 * a) if abs(a) > 1e-6 else 1.0
                    b = max(0.1, min(5, b))  # 限制b的范围
...
class FunctionVAE(nn.Module):
    def __init__(self, latent_dim=48):  # 增加潜在空间维度
        super().__init__()
...
def train_vae(model, train_loader, num_epochs=800, lr=0.01):
...
def sample_from_latent(model, num_samples=5):
    ...
    with torch.no_grad():
        ...
        for i in range(num_samples):
            ...
            # 修改为只使用正数范围
            x = np.linspace(-5, 5, 200)  # 确保使用正确的定义域
...
def main():
    ...
    # 修改文件名以反映新的函数类型
    dataset_file = 'Datasets/log_x_squared+1_dataset.pkl'
    ...
    save_model(model, 'Model_Library/log_x_squared+1_vae_model.pt')
