...
def generate_function():
    """ 生成 a * (b*x)^0.5 + c 形式的函数 """
    x = sp.Symbol('x')
    # 参数范围调整
    a = random.uniform(-2, 2)    # 控制函数的缩放
    b = random.uniform(0.1, 2)   # 必须为正数，控制x的缩放
    c = random.uniform(-2, 2)    # 控制垂直平移
    return a, b, c, a * (b * x)**(sp.Rational(1, 2)) + c  # 使用sp.Rational确保精确的1/2次方

def compute_l2_distance(f, g, domain=(0, 5)):
    x = sp.Symbol('x')
...
class CoefficientDataset(Dataset):
    def __init__(self, functions):
        ...
        for func in tqdm(functions):
            if isinstance(func, tuple):
                ...
                try:
                    ...
                    # 使用非负数范围的采样点
                    sample_points = np.linspace(0, 5, 50)  # 只使用正数范围
                    values = [float(func.subs(x, p)) for p in sample_points]
                    derivatives = [float(sp.diff(func, x).subs(x, p)) for p in sample_points]
                    
                    # 提取 a 和 b
                    # 对于 f(x) = a*(bx)^0.5 + c
                    # f'(x) = 0.5*a*b^0.5/x^0.5
                    
                    # 使用两点法估计a
                    x1, x2 = sample_points[1], sample_points[-1]  # 避免x=0点
                    y1, y2 = values[1] - c, values[-1] - c
                    a = (y2 - y1) / ((b*x2)**0.5 - (b*x1)**0.5)
                    
                    # 使用导数估计b
                    deriv_at_one = derivatives[len(derivatives)//2]
                    b = 4 * (deriv_at_one**2) / (a**2) if abs(a) > 1e-6 else 1.0
                    b = max(0.1, min(2, b))  # 限制b的范围
        ...
        for i, (param, (min_val, max_val)) in enumerate(self.ranges.items()):
            if param == 'b':
                # 对b使用对数归一化，因为它是正数且范围跨度较大
                self.coefficients[:, i] = (torch.log(self.coefficients[:, i]) - np.log(min_val)) / (np.log(max_val) - np.log(min_val))
            else:
                # 对a和c使用tanh归一化
...
def vae_loss(recon_x, x, mu, logvar, beta=0.1):
    ...
    recon_loss_b = F.mse_loss(recon_x[:, 1], x[:, 1]) * 3.0  # 增加b参数的权重
    recon_loss_c = F.mse_loss(recon_x[:, 2], x[:, 2])
    
    # 添加L1损失
    l1_loss = 0.1 * (torch.abs(recon_x - x).mean())
    
    # b参数的特殊约束：确保在[0,1]范围内，并且更倾向于较小的值
    b_constraint_loss = (
        F.relu(-recon_x[:, 1]).mean() +  # 下界约束
        F.relu(recon_x[:, 1] - 1).mean() +  # 上界约束
        0.1 * torch.sigmoid(recon_x[:, 1]).mean()  # 偏好较小的值
    )
...
def plot_reconstruction(model, dataset, num_samples=5, domain=(0, 5)):
    model.eval()
    with torch.no_grad():
...
def coefficients_to_function(coeffs):
    ...
    # 反归一化参数
    a = float(COEFF_STATS['a'][1] * np.arctanh(np.clip(coeffs[0], -0.99, 0.99)))
    
    # 对b使用指数反归一化
    b_min, b_max = COEFF_STATS['b']
    log_min, log_max = np.log(b_min), np.log(b_max)
    b = float(np.exp(log_min + np.clip(coeffs[1], 0, 1) * (log_max - log_min)))
    
    c = float(COEFF_STATS['c'][1] * np.arctanh(np.clip(coeffs[2], -0.99, 0.99)))
    
    return a * (b * x)**(sp.Rational(1, 2)) + c
...
def main():
    ...
    global COEFF_STATS
    COEFF_STATS = None
    num_functions = 2000  # Increased dataset size
    latent_dim = 8  # Reduced latent dimension
    num_epochs = 200
    batch_size = 64

    # 创建或加载数据集
    dataset_file = 'Datasets/sqrt_x_dataset.pkl'
    ...
    save_model(model, 'Model_Library/sqrt_x_vae_model.pt')
