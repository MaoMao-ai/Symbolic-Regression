...
def generate_function():
    """ 生成 a * cos(b * x^2) + c 形式的函数，使用更精确的参数生成 """
    x = sp.Symbol('x')
    # 直接返回参数和函数，避免信息丢失
    a = random.uniform(-2, 2)
    b = random.uniform(-3, 3)
    c = random.uniform(-2, 2)
    return a, b, c, a * sp.cos(b * x**2) + c
...
def main():
    ...
    # 创建或加载数据集
    dataset_file = 'Datasets/cos(x^2)_dataset.pkl'
    ...
    save_model(model, 'Model_Library/cos(x^2)_vae_model.pt')
