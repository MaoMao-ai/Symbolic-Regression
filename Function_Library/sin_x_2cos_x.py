...
def generate_function():
    """ 生成 a *sin(b*x^2)cos(c*x) + d 形式的函数，使用更精确的参数生成 """
    x = sp.Symbol('x')
    # 直接返回参数和函数，避免信息丢失
    ...
    return a, b, c, d, a * sp.sin(b * x**2) * sp.cos(c * x) + d
    ...
    def __init__(self, functions):
        ...
        for func in tqdm(functions):
            if isinstance(func, tuple):
                ...
                a, b, c, d, _ = func  # 修改为4个参数
                coeffs = [a, b, c, d]
            else:
                try:
                    # 提取偏移项 d
                    d = float(func.subs(x, 0))
                    ...
                    freqs = np.abs(np.fft.fftfreq(len(values)))
                    peaks = np.argsort(np.abs(fft))[-2:]  # 获取两个主要频率
                    b = freqs[peaks[0]] * 20
                    c = freqs[peaks[1]] * 20
        ...
        for coeffs in self.coefficients:
            # 原始数据
            augmented_coeffs.append(coeffs)
            
            # 添加小扰动
            noise = torch.randn_like(coeffs) * 0.01
            augmented_coeffs.append(coeffs + noise)
            
            # 参数符号翻转（保持函数形态）
            flipped = coeffs.clone()
            flipped[0] = -flipped[0]  # 翻转振幅
            augmented_coeffs.append(flipped)
        
        self.coefficients = torch.stack(augmented_coeffs)
...
def coefficients_to_function(coeffs):
    ...
    b = float(COEFF_STATS['b'][1] * np.arctanh(np.clip(coeffs[1], -0.99, 0.99)))
    c = float(COEFF_STATS['c'][1] * np.arctanh(np.clip(coeffs[2], -0.99, 0.99)))
    d = float(COEFF_STATS['d'][1] * np.arctanh(np.clip(coeffs[3], -0.99, 0.99)))
    
    return a * sp.sin(b * x**2) * sp.cos(c * x) + d
...
def main():
    ...
    # 创建或加载数据集
    dataset_file = 'Datasets/sin(x^2)cos(x)_dataset.pkl'
    ...
    save_model(model, 'Model_Library/sin(x^2)cos(x)_vae_model.pt')
